...
This directory keeps the completed E1 matched main-suite results, completed E2 DCLM, FineWeb-Edu, FineWeb, Dolma-sample, and C4 M0/300M result packages, the completed E8 learning-rate/weight-decay sensitivity study, and the WikiText demo anchor. Paper runs follow the manifest workflow below, and exact submitted commands are recorded in `ICLR_RUN_COMMANDS.md`.

MatrixPolicy values in the paper-facing tables, curves, token-savings readouts, runtime summaries, and sensitivity analysis use the 30 validated live-statistic-corrected main rows and 80 validated corrected E8 `rlb_fused_global_rational` rows under `corrections/matrixpolicy_live_stats_20260712/`. Non-MatrixPolicy RLB controls use the completed global-rational control sweep, and SiLU controls remain the fixed main-manifest controls.
...
corrections/matrixpolicy_live_stats_20260712/  # corrected main/E8 source
...
The corrected E8 MatrixPolicy sweep completed on 2026-07-15. Its manifest contains 80 rows over five datasets and the full $4\times4$ learning-rate/weight-decay grid; `corrections/matrixpolicy_live_stats_20260712/validation/e8.json` records `80/80` passes and no row errors. Paper generation combines only those corrected MatrixPolicy rows with the 160 unaffected SiLU controls from `manifests/iclr26_e8_primary_manifest.csv`.

MatrixPolicy is below each SiLU control at all 61 observed checkpoints in every matched cell. The minimum cellwise margins are `0.037904` against SiLU+AdamW and `0.042953` against SiLU+Muon. At the main target for each dataset, pooled median first-hit token savings are `15.7%` and `19.9%`, respectively. The corrected figure and target-arrival table are generated by `../paper/iclr_method_draft/generate_figures.py`.
