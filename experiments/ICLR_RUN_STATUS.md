...
Updated: 2026-06-22 20:28:38 EDT
...
Rejected V3/V4/V5/V7 proposal files and standalone manifests were deleted from the active repo surface on 2026-06-22; the V6 proposal-only artifact was also removed without a pilot. The V8 temporary schedule-pilot manifest was deleted after failed P0. The status summaries and raw run logs remain here as historical negative evidence.

## matrixpolicyV3 E1 Rerun Result
...
Decision: reject/supersede V3 and do not queue V3 E2. V3 kept the original MatrixPolicy mechanics and added partial horizontal gauge projection plus a confidence-gated Muon tail, but it did not improve E1 final loss on any dataset mean. The rejected proposal and manifest artifacts were removed from the active repo surface after V5 also failed.

| Field | Value |
| --- | --- |
| Proposal | removed after rejection; status retained here |
| Manifest | removed after rejection; raw run summaries retained under `experiments/runs/` |
...
| Runtime tracking | JSONL `summary.total_seconds`, `mean_seconds_per_step`, `tokens_per_second`, and train `optimizer_step_seconds`; not Slurm elapsed |

Aggregate result:

| Dataset | V3 final val loss | Original MatrixPolicy | Delta | V3 mean s/step | V3 tokens/s | V3 optimizer-step s |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| DCLM | 4.257245 +/- 0.003457 | 4.256224 | +0.001021 | 0.466493 | 70994.9 | 0.091257 |
| FineWeb-Edu | 4.089219 +/- 0.006443 | 4.088240 | +0.000979 | 0.564544 | 60624.0 | 0.091290 |
| FineWeb | 4.318981 +/- 0.009135 | 4.318581 | +0.000400 | 0.761884 | 58443.5 | 0.085624 |
| Dolma-sample | 4.324203 +/- 0.004118 | 4.323851 | +0.000352 | 1.097675 | 40623.5 | 0.087206 |
| C4 | 4.288422 +/- 0.015948 | 4.285119 | +0.003304 | 0.520759 | 66419.5 | 0.084667 |

Slurm accounting notes:

| Row | Job | Dataset seed | State | Restart note |
| ---: | ---: | --- | --- | --- |
| 5 | `690951` | FineWeb-Edu seed `3407` | completed `0:0` | slow full-step row on `fang-compute-02`; optimizer-step time remained normal |
| 7 | `690953` | FineWeb seed `2027` | completed `0:0` | `Restarts=1`; JSONL restarted cleanly and final summary is usable, but full-step throughput is contaminated |
| 9 | `690955` | Dolma-sample seed `1337` | completed `0:0` | slow full-step row on `monakhova-compute-01`; optimizer-step time remained normal |
| 11 | `690957` | Dolma-sample seed `3407` | completed `0:0` | slow full-step row on `monakhova-compute-01`; optimizer-step time remained normal |
...
Queued: 2026-06-21 13:29:07 EDT; replacement queued: 2026-06-21 13:34:08 EDT. Completed: 2026-06-21 16:53:52 EDT. Decision: reject/supersede V4 as a neutral result. V5 was later tested and rejected too.
...
The next tested optimizer-design proposal was V5, a joint function-space sensitivity metric over `(A_g, B_g)`. It is now completed and rejected below; the proposal artifact was removed from the active repo surface.

## matrixpolicyV5 E1 Joint Functional-Metric Run Status
Queued: 2026-06-21 17:06:55 EDT. Completed: 2026-06-21 22:37:04 EDT. Commit: `70233f9`. Decision: reject V5 and do not queue V5 E2.

V5 was not an engineering tweak. It changed the MatrixPolicy update metric for RLB matrix roles using joint function-space sensitivity over `(A_g, B_g)`. It did not change activation kernels, fusion, compiler flags, batching, launch shape, or cache behavior as a research contribution.
...
All submitted jobs `716298`-`716312` completed with exit `0:0`. Jobs `716298` and `716304` had `Restarts=1` from preemption/requeue and clean final JSONL summaries. The remaining jobs had `Restarts=0`.

Aggregate result:

| Dataset | V5 final val loss | Original MatrixPolicy | Delta | V5 mean runtime | V5 tokens/s |
| --- | ---: | ---: | ---: | ---: | ---: |
| DCLM | 4.257406 +/- 0.004169 | 4.256224 +/- 0.004972 | +0.001182 | 35.0 min | 52241.5 |
| FineWeb-Edu | 4.086836 +/- 0.007661 | 4.088240 +/- 0.009434 | -0.001404 | 29.8 min | 61612.5 |
| FineWeb | 4.319513 +/- 0.010019 | 4.318581 +/- 0.010914 | +0.000932 | 34.6 min | 52885.7 |
| Dolma-sample | 4.324036 +/- 0.005130 | 4.323851 +/- 0.004565 | +0.000185 | 29.7 min | 61794.9 |
| C4 | 4.285666 +/- 0.020566 | 4.285119 +/- 0.020677 | +0.000547 | 34.6 min | 52630.7 |

Per-seed final losses:

| Dataset | Seeds |
| --- | --- |
| DCLM | `1337:4.253464`, `2027:4.261770`, `3407:4.256984` |
| FineWeb-Edu | `1337:4.090235`, `2027:4.078063`, `3407:4.092209` |
| FineWeb | `1337:4.308059`, `2027:4.323828`, `3407:4.326651` |
| Dolma-sample | `1337:4.319663`, `2027:4.329682`, `3407:4.322763` |
...
## matrixpolicyV8 Fast-Pulse P0 Pilot
Queued: 2026-06-22 19:07:51 EDT. Completed: 2026-06-22 20:23:52 EDT. Decision: reject and prune. Do not queue V8 P1 or E1.

V8 was intentionally implemented with no new optimizer alias and no new source code path. It used existing `rational_matrix_policy_onpolicy` flags to test a method hypothesis: MatrixPolicy's expensive Muon component should act as an early matrix-geometry conditioning pulse, then hand off quickly to the cheaper AdamW matrix policy. The role-staged variant ended input-selector Muon earliest and allowed output-recombiner Muon slightly longer. This was a method schedule over the existing MatrixPolicy geometry, not a kernel/fusion/compiler tweak.

| Field | Value |
| --- | --- |
| Manifest | removed after failed P0; raw JSONL retained under `experiments/runs/iclr26_main/P0_matrixpolicyV8_fastpulse_500step/` |
| Phase | `P0_matrixpolicyV8_fastpulse_500step` |
| Dataset/seed | DCLM seed `1337` |
| Budget | `500` steps, `16,384,000` train tokens per row |
| Jobs | V1 control `727338`, fast pulse `727339`, lower-peak fast pulse `727340`; all completed `0:0`, `Restarts=0` |

Result:

| Metric | V1 control | V8 fast pulse | V8 lower peak |
| --- | ---: | ---: | ---: |
| Final val loss | 5.391057 | 5.395163 | 5.397042 |
| Validation-loss AUC | 6.368201 | 6.369571 | 6.370025 |
| Total seconds | 364.783 | 364.053 | 222.234 |
| Mean seconds/step | 0.695571 | 0.694920 | 0.413258 |
| Tokens/s | 47,109.5 | 47,153.6 | 79,291.9 |
| Slurm node | `ma-compute-01` | `ma-compute-01` | `barcheck-compute-01` |

Interpretation: the same-node fast-pulse row gave no material speedup and was worse on loss/AUC. The lower-peak row was much faster on a different node, but it was also worse on loss/AUC, so it cannot pass the P0 method gate. V8 suggests that simply shortening the Muon window damages the early matrix conditioning that V1 needs. The next candidate should not be another shorter-Muon schedule; it should preserve the original quality-critical Muon window while attacking cost through a mathematically justified lower-cost approximation or reuse of the matrix policy geometry.

## matrixpolicyV9 Approximate-Muon P0 Pilot
Queued: 2026-06-22 20:28:38 EDT. Decision status: pending on scheduler priority. Do not queue V9 P1 or E1 until this P0 passes the paired loss/runtime gate.

V9 is implemented with no new optimizer alias and no new source code path. It keeps the original MatrixPolicy schedule that V8 showed is quality-critical, but reduces Muon Newton-Schulz iterations inside that same window. The method hypothesis is that RLB matrix conditioning may need a sign/polar-like Muon direction, not a high-accuracy orthogonalization; a lower-accuracy NS approximation could preserve the conditioning effect while reducing matrix-update cost.

| Field | Value |
| --- | --- |
| Temporary manifest | `experiments/manifests/iclr26_matrixpolicyV9_approx_muon_p0_manifest.csv` |
| Phase | `P0_matrixpolicyV9_approx_muon_500step` |
| Dataset/seed | DCLM seed `1337` |
| Budget | `500` steps, `16,384,000` train tokens per row |
| Row 0 | V1 control, job `727913`, method `rlb_matrixpolicy_original` |
| Row 1 | V9 NS=3, job `727914`, method `rlb_matrixpolicyV9_ns3` |
| Row 2 | V9 NS=2, job `727915`, method `rlb_matrixpolicyV9_ns2` |
| Estimated starts | `727913` at `2026-06-22T20:33:55`, `727914` at `2026-06-22T22:34:00`, `727915` at `2026-06-23T00:34:00` |

P0 gate: a candidate must match or improve paired V1 final loss/AUC while improving runtime, tokens/s, or optimizer-step time. If both V9 rows fail, delete the temporary manifest and retain only a concise failed-pilot record here.
...
Current check: 2026-06-19 14:33:01 EDT. E2 M0/300M is complete for DCLM rows `240-284`, FineWeb-Edu rows `285-329`, FineWeb rows `330-374`, Dolma-sample rows `375-419`, and C4 rows `420-464`. Rows `465+` are E3 and were not queued. Live scheduler state at update: no active `mt872` jobs in `squeue`; active allocation at update is 0 A6000 total.
