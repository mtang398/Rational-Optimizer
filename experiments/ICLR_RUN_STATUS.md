...
Updated: 2026-06-24 11:53:46 EDT
...
Rejected MatrixPolicy V2-V12 artifacts were pruned from the active repo surface and raw run tree on 2026-06-23. The single retained negative-result state is `optimizer_design/proposals/matrixpolicy_variant_failures.md`; this status file keeps only the accepted safe-speed MatrixPolicy records and paper-facing completed-suite summaries.

## Rational-Only RLB Ablation
Queued: 2026-06-24 11:40:53 EDT. Corrected/requeued: 2026-06-24 11:46 EDT. Stopped: 2026-06-24 11:50:32 EDT. Decision: failed negative ablation; do not count as completed E1 or E2.

Purpose: isolate the value of the RLB local basis. Activation `rlb_fused_rational_only_ffn` keeps the same single-branch grouped RLB block and the same MatrixPolicy optimizer path, but uses no local rational atoms: `centers=()`, `beta` shape `[groups, 0]`, and `coeff_logits` shape `[groups, 0, 2]`. The scalar nonlinearity is only the grouped Version-A P5/Q4 rational initialized from the SiLU fit. Original `rlb_fused_fixed_strong_ffn` remains the paper anchor.

Implementation and safety updates:

| Field | Value |
| --- | --- |
| Manifest | `experiments/manifests/iclr26_rational_only_ablation_manifest.csv` |
| Builder | `experiments/scripts/build_iclr26_rational_only_ablation_manifest.py` |
| Activation | `rlb_fused_rational_only_ffn` |
| Method | `rlb_rational_only_matrixpolicy` |
| Optimizer | `rational_matrix_policy_onpolicy`, same MatrixPolicy extra args as `rlb_matrixpolicy_original` |
| Planned coverage | E1 100M and E2 300M, 5 datasets x 3 seeds each, 30 rows total |
| Timing guard | phases `E1_rational_only_100m` and `E2_rational_only_300m` require an `nvlink` node before JSONL archive/write, unless `ALLOW_NON_NVLINK_TIMING=1` |
| Harness safety | `training/transformer_lm_compare.py` now writes `stopped_early` and exits on nonfinite train or validation loss without requiring a configured threshold |

Submission audit:

| Jobs | State | Note |
| --- | --- | --- |
| `821379` | Slurm `COMPLETED 0:0`, excluded | Extension build succeeded, but torchrun rejected `rlb_fused_rational_only_ffn` because argparse choices were missing; produced no valid training row |
| `821380`-`821408` | cancelled | Old dependent chain cancelled after the parser issue was found; `821380` and `821381` only ran for about six seconds and produced no valid row |
| `821439` | cancelled after 3:13 | Clean replacement row 0, DCLM E1 seed `1337`; finite at step `50` with val loss `8.145039`, train loss nonfinite at step `70`, validation nonfinite at step `100` |
| `821440` | cancelled after 3:13 | Clean replacement row 1, DCLM E1 seed `2027`; finite at step `50` with val loss `8.129251`, train loss nonfinite at step `70`, validation nonfinite at step `100` |
| `821441`-`821468` | cancelled before start | Remaining E1/E2 rows were dependency-pending and were stopped to avoid repeating a confirmed invalid method |

Early speed diagnostics before divergence were in the expected fused-code band, roughly `0.38`-`0.42` s/step and `78k`-`86k` tokens/s on the two NVLink allocations. These are not valid runtime benchmark rows because both runs diverged almost immediately.

Interpretation: removing the local rational atoms is not just removing overhead. Under the same MatrixPolicy schedule, the rational-only P5/Q4 path loses the stabilizing local basis statistics and the matrix-policy telemetry becomes nonfinite almost immediately. The negative result supports keeping the local basis in the paper anchor and using rational-only only as a failed ablation record.
