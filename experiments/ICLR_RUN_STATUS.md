...
Updated: 2026-06-22 19:15:48 EDT
...
Rejected V3/V4/V5/V7 proposal files and standalone manifests were deleted from the active repo surface on 2026-06-22; the V6 proposal-only artifact was also removed without a pilot. The status summaries and raw run logs remain here as historical negative evidence.
...
Queued: 2026-06-22 19:07:51 EDT. Decision status: pending on scheduler priority as of 2026-06-22 19:15:48 EDT. Do not queue P1 or E1 until this P0 passes the paired loss/runtime gate.

V8 is intentionally implemented with no new optimizer alias and no new code path. It uses existing `rational_matrix_policy_onpolicy` flags to test a method hypothesis: MatrixPolicy's expensive Muon component should act as an early matrix-geometry conditioning pulse, then hand off quickly to the cheaper AdamW matrix policy. The role-staged variant ends input-selector Muon earliest and allows output-recombiner Muon slightly longer. This is a method schedule over the existing MatrixPolicy geometry, not a kernel/fusion/compiler tweak.

| Field | Value |
| --- | --- |
| Temporary manifest | `experiments/manifests/iclr26_matrixpolicyV8_fastpulse_p0_manifest.csv` |
| Phase | `P0_matrixpolicyV8_fastpulse_500step` |
| Dataset/seed | DCLM seed `1337` |
| Budget | `500` steps, `16,384,000` train tokens per row |
| Validation cache | existing `4,000,000` tokens after skip `210,000,000` |
| Row 0 | V1 control, job `727338`, method `rlb_matrixpolicy_original` |
| Row 1 | V8 role-staged fast pulse, job `727339`, method `rlb_matrixpolicyV8_fastpulse` |
| Row 2 | V8 lower-peak fast pulse, job `727340`, method `rlb_matrixpolicyV8_fastpulse_low` |

P0 gate: a candidate must match or improve paired V1 final loss/AUC while improving runtime or tokens/s. If both V8 rows fail, delete the temporary manifest and retain only a concise failed-pilot record here.
