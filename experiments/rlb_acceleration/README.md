...
The current paper-facing MatrixPolicy result rows are now the corrected global-rational/no-local-atom (`rlb_fused_global_rational`) replacement rows. On E1, the global-rational MatrixPolicy aggregate is `24.5` min, `0.4563` s/step, and `72,344.1` tokens/s over 15 rows, versus the earlier local-atom (`rlb_fused_fixed_strong_ffn`) safe-speed aggregate of `27.3` min, `0.5102` s/step, and `67,078.3` tokens/s. That is about a 10% mean runtime reduction on E1.
