...
The current paper-facing MatrixPolicy rows use the validated live-statistic-corrected `rlb_fused_global_rational` campaign. The corrected E1 aggregate is `24.9` min, `0.4670` s/step, and `70,774.8` tokens/s over 15 rows. The corrected E2 per-dataset means are DCLM `72.0` min, FineWeb-Edu `79.3` min, FineWeb `74.5` min, Dolma-sample `76.4` min, and C4 `74.1` min.

The corrected 300M rows ran on multiple node classes. `lancer-compute-01` rows averaged about `69.0` min, whereas `ellis-compute-02` rows averaged about `82.4` min. These allocation differences confound comparisons with the earlier safe-speed campaign, so this worklog no longer attributes the corrected-versus-old runtime delta to the activation or optimizer. The canonical runtime tables report every observed row without exclusion.
