#!/usr/bin/env python3
"""Build an incremental or final result ledger for the TILLER matrix."""
...
def matrix_payload():
    ...
    if payload.get("matrix_rows") != len(payload.get("rows", ())):
...
def main() -> None:
    ...
    if args.require_complete and terminal_count != matrix_rows:
        raise RuntimeError(
    ...
    for (model, dataset), rows in sorted(grouped.items()):
        ...
        summaries.append({
            ...
            "exact_matched_hardware_endpoint_total_time_ratio": (
                sum(float(row["candidate_end_to_end_total_seconds"]) for row in rows)
                / sum(float(row["control_end_to_end_total_seconds"]) for row in rows)
            ),
    ...
    result = {
        "schema": "tiller_results_v1",
        "status": (
            "complete" if terminal_count == matrix_rows else "running"
        ),
        ...
        "all_dataset_seed_rows_terminal": terminal_count == matrix_rows,
