...
from .conditions import CONDITIONS, GRAIN_CONDITIONS
...
from .model import ModernMHADecoder, copy_shared
...
def initial_model(cfg, condition):
    ...
    base = ModernMHADecoder(cfg['model'])
    if condition in GRAIN_CONDITIONS:
        model = ModernMHADecoder(cfg['model'], grain=True, groups=cfg['grain']['groups'])
        copied = copy_shared(base, model)
    else:
        model = base
        copied = [name for name, _ in model.named_parameters() if '.mlp.' not in name]
    h = hashlib.sha256()
...
def validate_config(cfg):
    ...
    if cfg['grain']['intermediate_size']*2 != c['intermediate_size']*3 or cfg['grain']['intermediate_size'] != cfg['grain']['groups']*cfg['grain']['group_width']:
        raise ValueError('GRAIN/SwiGLU width matching differs')
...
def runtime_identity(device, condition):
    ...
    for module, directory in ((rational_opt, 'activation'), (core, 'optimizer_design')):
        if not Path(module.__file__).resolve().is_relative_to(root/directory):
            raise RuntimeError('optimizer/activation imported from another checkout')
    result = {'packages':{name:importlib.metadata.version(name) for name in ('torch','numpy','transformers','tokenizers')},
              'precision':'bf16_autocast_fp32_master' if device.type == 'cuda' else 'cpu_fp32_reference',
              'cuda_toolkit':torch.version.cuda}
    if condition in GRAIN_CONDITIONS and device.type == 'cuda':
        from rational_opt import _C
        if not Path(_C.__file__).resolve().is_relative_to(root/'activation'): raise RuntimeError('extension imported from another checkout')
        result['extension_sha256'] = digest(_C.__file__)
    return result
...
def main():
    ...
    if device.type == 'cuda':
        ...
        if os.environ.get('RATIONAL_OPT_TORCH_FALLBACK', '0') in ('1', 'true', 'True', 'yes'): raise RuntimeError('CPU rational fallback is forbidden for CUDA training')
        if args.condition in GRAIN_CONDITIONS:
            from rational_opt import _C  # Require the compiled extension; no silent approximation.
    ...
    model = model.to(device); optimizer = build_optimizer(model, args.condition, cfg['conditions'][args.condition]['lr'], cfg['accumulation'])
    completed = 0
    if args.resume:
        saved = json.loads((args.output/'latest.json').read_text())['manifest'] if args.resume == 'latest' else args.resume
        completed = checkpoint.load(saved, model, optimizer, identity, lambda u: batch_hash(data, u, rank))
    ...
    while completed < cfg['updates']:
        ...
        if optimizer.router is not None and completed % 100 == 0:
            metrics['tiller_telemetry'] = optimizer.router.telemetry()
            metrics['attention_telemetry'] = optimizer.attention.telemetry()
        if rank == 0:
        ...
        boundary = args.condition == 'grain_tiller100_muon' and completed in (100, 101)
