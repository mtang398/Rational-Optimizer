...
response summaries, and an idealized positive matrix-pair rescaling degree of
freedom under the implemented RMS floor.  We then introduce MatrixPolicy, a
...
at useful loss levels.  Across five 12-layer, width-768 language-model
pretraining corpora, MatrixPolicy reaches hard common validation-loss targets
12.0--15.3M tokens earlier than SiLU with AdamW and 20.8--27.9M tokens earlier than
SiLU with Muon in the 100M-token E1 regime; in the 300M-token E2 regime it reaches
those targets 51.9--69.9M tokens earlier than SiLU with AdamW and 13.1--19.7M tokens
earlier than SiLU with Muon.  Using measured throughput summaries, every dataset and
comparator pair also shows positive estimated training-time savings.
\end{abstract}
...
\subsection{Main Result: Faster Target Arrival}
\label{sec:main-result-target-arrival}

\input{tables/e1_e2_silu_summary_table.tex}

Table~\ref{tab:e1e2-silu-comparison} is the primary empirical result.  It asks a target-arrival question: for the hardest common
validation-loss target reached by every listed method in all three seeds, how
many training tokens and how much measured training time are required?  Across
all five E1 datasets, MatrixPolicy reaches the common target 12.0--15.3M tokens
earlier than SiLU with AdamW and 20.8--27.9M tokens earlier than SiLU with Muon.
The same table shows that the gain is not only an activation replacement effect:
using the global rational activation with AdamW or Muon still requires more
tokens than MatrixPolicy at the same targets.  In E2 the endpoint losses are
closer after longer training, but MatrixPolicy still reaches the common targets
earlier than every listed comparator in both tokens and measured wall-clock time.

\begin{figure*}[t]
\centering
\includegraphics[width=0.99\textwidth]{figures/target_arrival_evidence_matrix.pdf}
\caption{Dense target-arrival summary for the completed E1/E2 study.  Each row is
one dataset/regime target from Table~\ref{tab:e1e2-silu-comparison}.  Bubble area
encodes tokens saved by MatrixPolicy at the common target, fill color encodes
training time saved, and the right block gives the endpoint validation-loss gap
(comparator minus MatrixPolicy).  Positive values favor MatrixPolicy.}
\label{fig:target-arrival-evidence}
\end{figure*}

Figure~\ref{fig:target-arrival-evidence} gives the same result as a dense visual
check.  The bubble matrix shows that all comparator cells have positive token and
time savings, while the endpoint-gap block shows that the earlier arrival does
not come from stopping at a worse final model within these fixed-scale runs.

\subsection{Training Dynamics Support the Target-Arrival Result}
...
Table~\ref{tab:e1e2-silu-comparison} and Figures~\ref{fig:target-arrival-evidence}--\ref{fig:e1-representative-dynamics}.
