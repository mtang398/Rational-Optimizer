...
response summaries, and an approximately homogeneous matrix-pair degree of
freedom under the implemented RMS floor.  We then introduce MatrixPolicy, a
role-aware optimizer for the RLB input and output matrices.  In the paper-facing
...
12.0--15.3M tokens earlier than SiLU+AdamW and 20.8--27.9M tokens earlier than
SiLU+Muon in the 100M-token E1 regime; in the 300M-token E2 regime it reaches
those targets 51.9--69.9M tokens earlier than SiLU+AdamW and 13.1--19.7M tokens
earlier than SiLU+Muon.  Using measured throughput summaries, every dataset and
...
substep, and keep the input/output representatives balanced.  In the paper-facing
...
\begin{itemize}
    ...
    matrices in a paper-facing configuration where rational coefficients and
...
matrix-aware optimization, but in the paper-facing configuration it applies
...
non-module quantities such as $c_{l,g,r}$, $\mu_{l,r,t}$, $\gamma_{l,g}$,
...
Transformer on five corpora: DCLM/DataComp-LM \citep{li2024datacomp},
FineWeb-Edu and FineWeb \citep{penedo2024fineweb}, Dolma-sample
\citep{soldaini2024dolma}, and C4 \citep{raffel2020t5}.  E1 trains for
approximately 100M tokens and E2 for approximately 300M tokens, with three seeds
per method/dataset pair.  The main comparison is deliberately narrow:
MatrixPolicy against the two standard SiLU baselines, AdamW and Muon.  Shared
architecture, optimizer, evaluation-cadence, and throughput-measurement details
are reported in Appendix~\ref{app:experiment-protocol}; broader RLB optimizer
controls are also moved there.

\input{tables/e1_e2_silu_summary_table.tex}

Table~\ref{tab:e1e2-silu-comparison} summarizes the main target-arrival result.
For each dataset and regime, the target is the hardest pre-specified
validation-loss level reached by MatrixPolicy, SiLU+AdamW, and SiLU+Muon in all
three seeds.  Positive $\Delta$tok and $\Delta t$ mean MatrixPolicy reaches that
same loss earlier.  In E1, MatrixPolicy saves 12.0--15.3M tokens against
SiLU+AdamW and 20.8--27.9M tokens against SiLU+Muon.  In E2, where endpoint loss
separation is more saturated, the target-arrival savings remain positive:
51.9--69.9M tokens against SiLU+AdamW and 13.1--19.7M tokens against SiLU+Muon.
The time column uses measured throughput summaries rather than assuming equal
steps/second.

\begin{figure*}[t]
\centering
\includegraphics[width=0.99\textwidth]{figures/e1_multimetric_all_datasets.pdf}
\caption{E1 trajectories across all five datasets.  Rows show validation loss,
validation perplexity, and training loss; columns show datasets.  Lines are seed
means over three runs and bands show one sample standard deviation.  E1 is the
most revealing completed regime because long-budget saturation has not yet
compressed the optimization-speed separation.}
\label{fig:e1-multimetric-all}
\end{figure*}

Figure~\ref{fig:e1-multimetric-all} shows that the target-arrival advantage is
not a single-threshold artifact.  MatrixPolicy separates early in validation loss
and validation perplexity while also keeping training loss competitive.  We use
E1 as the main trajectory view because a fixed long budget makes strong
activation/optimizer pairs increasingly close by the endpoint; the paper claim
is therefore about reaching useful losses with fewer tokens and less estimated
training time, with endpoint loss used as supporting evidence.

\begin{figure*}[t]
\centering
\includegraphics[width=0.99\textwidth]{figures/combined_result_dotplot.pdf}
\caption{Loss-token target plane for the completed E1/E2 comparisons.  Each
polyline connects pre-specified validation-loss targets for one dataset/method
pair; lower token position at the same target indicates faster arrival.  Color
encodes method, marker shape encodes dataset, and panel encodes the E1 or E2
regime.}
\label{fig:combined-dotplot}
\end{figure*}

Figure~\ref{fig:combined-dotplot} puts the table targets back into the broader
candidate-target grid.  Across both regimes, the MatrixPolicy curves sit to the
left of the SiLU baselines for the reachable target range, showing that the main
result is a consistent movement of the loss-token frontier rather than one
selected row of Table~\ref{tab:e1e2-silu-comparison}.

The present main text does not claim results from unrun larger-scale experiments
or unrun component ablations.  Those studies are intentionally left outside the
current evidence package; Appendix~\ref{app:experiment-protocol} records the
completed controls and supporting curves that already exist.
...
the RLB input and output matrices with role-aware statistics in the
paper-facing configuration, while the remaining parameter groups use AdamW.  The fixed-scale language-model experiments show a
...
DCLM and FineWeb-Edu.  These panels support the main multi-metric view in
Figure~\ref{fig:e1-multimetric-all}.}
\label{app:e1-curves}
\end{figure}
