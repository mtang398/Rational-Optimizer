...
\begin{abstract}
We improve Transformer pretraining by using learned activation responses
to coordinate matrix updates. GRAIN, \textbf{G}roupwise \textbf{R}ational
\textbf{A}ctivation with \textbf{I}nternal \textbf{N}ormalization, learns
rational nonlinearities on normalized feature groups and restores the
group scale at its output. TILLER, \textbf{T}angent-\textbf{I}nformed
\textbf{L}oss \textbf{L}edger for \textbf{E}qual-budget
\textbf{R}eweighting, uses the resulting features and their derivatives
to orient matrix update directions. A compact loss ledger tracks how
their first-order loss responses reinforce or offset one another at shared
token positions. TILLER combines this history with current gradients to
allocate update strength jointly across groups and layers. At matched
training-token budgets, GRAIN with TILLER achieves the lowest mean
validation loss among the evaluated adaptive and matrix-based optimizer
baselines across every tested corpus and model size.
\end{abstract}
