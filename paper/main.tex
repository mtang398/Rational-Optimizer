...
\begin{abstract}
A learned activation can guide how a network learns as well as what it
represents. We design the activation and optimizer together for Transformer
pretraining, using the activation's evolving response to coordinate matrix
updates across groups and layers. \textbf{GRAIN}, \textbf{G}roupwise \textbf{R}ational
\textbf{A}ctivation with \textbf{I}nternal \textbf{N}ormalization, learns
group-specific rational nonlinearities in normalized feature coordinates
and restores the group scale at its output. \textbf{TILLER}, \textbf{T}angent-\textbf{I}nformed
\textbf{L}oss \textbf{L}edger for \textbf{E}qual-budget \textbf{R}eweighting,
uses the learned features and their derivatives to orient matrix directions
and measure their first-order loss responses at token positions shared across layers.
These measurements reveal how the loss responses of different directions
reinforce or offset one another, relationships not determined by their
average descent scores. A compact loss ledger retains this joint response
history as the activations and update directions evolve. TILLER combines
the descent predicted by current gradients with this response history
to allocate update strength jointly across groups and layers, preserving
the combined norm of the feed-forward directions at each step. At matched
training-token budgets, GRAIN with TILLER achieves lower mean validation loss
than AdamW, Muon, and SOAP across the tested corpora and model sizes,
including comparisons in which those optimizers use the same GRAIN activation.
\end{abstract}
