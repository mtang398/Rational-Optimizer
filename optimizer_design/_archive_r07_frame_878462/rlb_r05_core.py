...
class R05Core(R08RevisionCore):
    ...
    def step(self, closure=None):
        ...
        if self._capture_telemetry_next_step:
            ...
            diagnostic_descents = torch.stack(
                (
                    incoming_ordinary_descent,
                    outgoing_ordinary_descent,
                    incoming_adaptive_descent,
            ...
            if (
                torch.any(~torch.isfinite(alignments))
                or torch.any(alignments < 0.0)
                or torch.any(alignments > 1.0)
                or torch.any(~torch.isfinite(diagnostic_descents))
                or torch.any(~torch.isfinite(incoming_descent))
                or torch.any(~torch.isfinite(outgoing_descent))
        ...
        self._apply_matrix_update(
            incoming_direction, outgoing_direction, lr, weight_decay
        )
    ...
    def _apply_matrix_update(
        self, incoming_direction, outgoing_direction, lr, weight_decay
    ):
        """Commit one paired matrix update; subclasses may stage it atomically."""
