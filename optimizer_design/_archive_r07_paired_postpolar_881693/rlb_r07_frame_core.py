...
class R07PairedAdaptiveFrameCore(R07FrameCore):
    """R07 frame polar with paired post-polar neuron normalization.

    The extra state is one matched-beta2 second moment for every canonical
    incoming-row/outgoing-row RLB channel pair.  It is measured from the
    already polarized R07 frame source, so it is downstream of the joint
    cross-role spectral decision and upstream of literal-parent
    residualization and exact budget closure.
    """

    component_code = 48
    checkpoint_schema = "r07_r03_cross_role_frame_paired_adaptive_v2"
    inherited_parent = "complete_r07_cross_role_frame_polar"
    new_scientific_components = R07FrameCore.new_scientific_components + (
        "paired_postpolar_second_moment_normalization",
    )
    ...
    def __init__(
        ...
        pairs,
        *,
        use_paired_postpolar_adaptive: bool = True,
        **kwargs,
    ):
        if type(use_paired_postpolar_adaptive) is not bool:
            raise ValueError("R07 paired post-polar flag must be boolean")
        self.use_paired_postpolar_adaptive = use_paired_postpolar_adaptive
        self._r07_pair_adaptive_metadata = None
        super().__init__(pairs, **kwargs)
        if float(self._r05_beta2) != 0.95 or float(self._r05_eps) != 1.0e-8:
            raise ValueError("R07 paired adaptation must use cell beta2/epsilon")

    def lr_wd_fairness_audit(self):
        report = super().lr_wd_fairness_audit()
        report.update({
            "paired_postpolar_second_moment_lr_scale": 1.0,
            "paired_postpolar_second_moment_weight_decay_scale": 1.0,
            "paired_postpolar_parent_budget_lr_scale": 1.0,
        })
        return report
    ...
    def _coefficient_of_variation(values):
        mean = values.mean()
        return values.std(unbiased=False) / mean.clamp_min(
    ...
    def _frame_source(
        ...
        *,
        ...
        incoming, outgoing, metadata = super()._frame_source(
            incoming_momentum,
            outgoing_momentum,
            ns_steps=ns_steps,
        ...
        if not self.use_paired_postpolar_adaptive:
            scalar = incoming.new_zeros(())
            self._r07_pair_adaptive_metadata = {
                "enabled": False,
                "step": 0,
                "pair_energy_cv": scalar,
                "persistent_energy_cv": scalar,
                "row_scale_cv": scalar,
                "normalized_pair_energy_cv": scalar,
                "row_scale_min": incoming.new_ones(()),
                "row_scale_max": incoming.new_ones(()),
            }
        ...
        self._r07_pair_adaptive_metadata = {
            "enabled": True,
            "step": next_step,
            "pair_energy_cv": self._coefficient_of_variation(pair_energy),
            "persistent_energy_cv": self._coefficient_of_variation(
                persistent_energy
            ),
            "row_scale_cv": self._coefficient_of_variation(row_scale),
            "normalized_pair_energy_cv": self._coefficient_of_variation(
                normalized_pair_energy
            ),
            "row_scale_min": row_scale.amin(),
            "row_scale_max": row_scale.amax(),
        }
        return incoming, outgoing, metadata
    ...
    def step(self, closure=None):
        ...
        self._r07_pair_adaptive_metadata = None
        loss = super().step(closure)
        metadata = self._r07_pair_adaptive_metadata
        if metadata is None:
            raise RuntimeError("R07 paired post-polar state did not execute")
        if publish:
            self._last_telemetry.update({
                "rlb_r07_component_code": self.component_code,
                "rlb_r07_parent_is_complete_frame_r07": 1,
                "rlb_r07_paired_postpolar_second_moment_enabled": int(
                    metadata["enabled"]
                ),
                "rlb_r07_paired_postpolar_beta2": float(self._r05_beta2),
                "rlb_r07_paired_postpolar_eps": float(self._r05_eps),
                "rlb_r07_paired_postpolar_step": int(metadata["step"]),
                "rlb_r07_pair_energy_cv": float(
                    metadata["pair_energy_cv"].item()
                ),
                "rlb_r07_persistent_pair_energy_cv": float(
                    metadata["persistent_energy_cv"].item()
                ),
                "rlb_r07_pair_row_scale_cv": float(
                    metadata["row_scale_cv"].item()
                ),
                "rlb_r07_normalized_pair_energy_cv": float(
                    metadata["normalized_pair_energy_cv"].item()
                ),
                "rlb_r07_pair_row_scale_min": float(
                    metadata["row_scale_min"].item()
                ),
                "rlb_r07_pair_row_scale_max": float(
                    metadata["row_scale_max"].item()
                ),
            })
        return loss
...
__all__ = ("R07FrameCore", "R07PairedAdaptiveFrameCore")
