...
Status: active pilot policy. `matrixpolicyV7`, `matrixpolicyV8_fastpulse`, and `matrixpolicyV9_approx_muon` P0 all failed on 2026-06-22 and were pruned from the active repo surface. V7 slightly improved loss but was slower; V8 tested no-new-code shorter Muon pulse schedules and worsened loss/AUC; V9 tested lower Muon Newton-Schulz accuracy inside the original MatrixPolicy window and worsened loss/AUC for only negligible same-node speed. Current active use: method-preserving safe-speed P0 jobs `727990`-`727992`, validating commit `02b85d9`, plus V10 switch-clean P0 jobs `728006`-`728007`. V10 keeps full-quality early MatrixPolicy conditioning and tests whether resetting matrix Adam state at the permanent Muon-off switch improves late Adam geometry.
