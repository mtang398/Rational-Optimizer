...
Current E1 M0/100M manifest-suite readout, with MatrixPolicy taken from the validated live-statistic-corrected campaign:
...
The method-preserving Muon-off speed fix remains in production: once every MatrixPolicy Muon group has passed its decay end and `final_muon=min_muon=0`, `RationalMatrixPolicyOptimizer` skips the otherwise zero-LR Muon step. The current paper-facing rows also use training-only, globally synchronized RLB sufficient statistics and `rlb_fused_global_rational`. The corrected E1 aggregate is `24.9` min, `0.4670` s/step, and `70,774.8` tokens/s over 15 rows. Corrected 300M rows span multiple node classes, so the generated runtime tables report observed allocation-specific time and do not support a hardware-normalized speed comparison.
