#!/usr/bin/env python3
"""Run-local exact-resume support for matched distributed experiments.

Each checkpoint binds the complete argument namespace and every rank's state.
The recovery path completes an interrupted atomic commit and trims JSONL
records beyond the newest durable checkpoint before replaying that step range.
"""
...
def install(trainer) -> None:
    """Install run-local resume validation and checkpoint hooks."""
    ...
    def strict_validate(args) -> None:
        ...
        # Run the standard protocol checks with the validated local path
